    ...
    def createLoss(self, model):
        ...
        with tf.variable_scope("Loss"):
            ...
            if self.crf is True:
                ...
                all_loss = self._computeSentenceLevelLoss(gold, mask, lengths, model)
            else:
                print('crf=False, creating WLL')
                all_loss = self._computeWordLevelLoss(gold, mask, model)
        return all_loss

    def _computeSentenceLevelLoss(self, gold, mask, lengths, model):

        ll, self.A = tf.contrib.crf.crf_log_likelihood(model.probs, model.y, lengths)
        all_total = tf.reduce_sum(lengths, name="total")
        return tf.reduce_mean(-ll)

    def _computeWordLevelLoss(self, gold, mask, model):

        nc = len(model.labels)
        # Cross entropy loss
        print(model.probs.get_shape())
        print(tf.one_hot(model.y, nc, axis=-1).get_shape())
        cross_entropy = tf.one_hot(model.y, nc, axis=-1) * tf.log(tf.nn.softmax(model.probs))
