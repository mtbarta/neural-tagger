...
class DConv(SequentialModel):
    ...
    def attention(self, input_mat, num_filt, i, reuse=False):
    	"""
		advanced attentional convolution as described in https://arxiv.org/pdf/1710.00519.pdf
    	"""

    	attention_mat = tf.Variable(tf.truncated_normal(input_mat.get_shape(),
                                                stddev = 0.1), name="attn-"+str(i))
    	attention_source = multigranular_gating(input_mat, 
    											num_filt, 
    											i, 
    											reuse=reuse, 
    											granularities=[1,3], 
    											name="attn_source")
    	attention_focus = multigranular_gating(attention_mat, 
    											num_filt, 
    											i, 
    											reuse=reuse, 
    											granularities=[1,3], 
    	...
    	attention_beneficiary = gated_convolution(input_mat,
    											  num_filt,
    											  i,
    											  reuse=reuse,
    											  kernel_width=1)

    	energy_mat = tf.matmul(attention_source, attention_focus)
    	energy_activation = tf.nn.softmax(energy_mat, dim=2)

    	context = tf.matmul(energy_activation, attention_beneficiary)
    	context_feature_map = tf.reduce_sum(context, axis=1)

    	return context_feature_map


    def multigranular_gating(self, input_mat, num_filt, i, reuse=False, granularities=[1,3], name="multigranular_gate"):
    	"""
		concatenate multiple gated_convolutions with different convolutional kernels.
    	"""
    	mgran = []
    	with tf.variable_scope(name, reuse=reuse):
    		for granularity in granularities:
    			res = gated_convolution(input_mat, num_filt, i, reuse=reuse, kernel_width=granularity)
    			mgran.append(res)


    def gated_convolution(self, input_tensor, num_filt, i, reuse=False, kernel_width=1):
    	"""
    	...
    	with tf.variable_scope('gated_conv', reuse=reuse):
    		conv = tf.layers.conv1d(input_tensor, num_filt, kernel_width, 
                                                    padding='same', 
                                                    dilation_rate=1, 
                                                    activation=tf.tanh,
                                                    kernel_initializer=tf.orthogonal_initializer(), #tf.contrib.layers.xavier_initializer(),
    		...
    		gate = tf.layers.conv1d(input_tensor, num_filt, kernel_width, 
                                                    padding='same', 
                                                    dilation_rate=1, 
